...
describe('BasaltDataFilter', (): void => {
    describe('filterByKeys', (): void => {
        ...
        it('should return an object with filtered keys', (): void => {
            ...
            const result: { a: number; b: number; c: number } = BasaltDataFilter.filterByKeys(data, ['a', 'c']);
            expect(result).toEqual({ a: 1, c: 3 });
        });

        it('should return an empty object when no keys match', (): void => {
            const data: { a: number; b: number; c: number } = { a: 1, b: 2, c: 3 };
            const result: { a: number; b: number; c: number } = BasaltDataFilter.filterByKeys(data, []);
        ...
        it('should exclude keys with null or undefined values when excludeNullUndefined option is true', (): void => {
            ...
            const result: { a: number; b: number; c: number | null; d?: number } = BasaltDataFilter.filterByKeys(data, ['a', 'b', 'c', 'd'], { excludeNullUndefined: true });
            expect(result).toEqual({ a: 1, b: 2 });
        });

        it('should include keys with null or undefined values when excludeNullUndefined option is false or not provided', (): void => {
            const data: { a: number; b: number; c: number | null; d?: number } = { a: 1, b: 2, c: null };
            const result: { a: number; b: number; c: number | null; d?: number } = BasaltDataFilter.filterByKeys(data, ['a', 'b', 'c', 'd']);
    ...
    describe('excludeByKeys', (): void => {
        it('should throw an error for null data', (): void => {
            expect((): void => {
                BasaltDataFilter.excludeByKeys(null as any, ['a', 'b', 'c']);
            }).toThrowError('The provided data object is either null or undefined.');
        });

        it('should throw an error for undefined data', (): void => {
            expect((): void => {
                BasaltDataFilter.excludeByKeys(undefined as any, ['a', 'b', 'c']);
            }).toThrowError('The provided data object is either null or undefined.');
        });

        it('should throw an error if the data is not an object', (): void => {
            expect((): void => {
                BasaltDataFilter.excludeByKeys('string' as any, ['a', 'b', 'c']);
            }).toThrowError('The provided data object must be an object.');
        });

        it('should throw an error if the keys are not an array', (): void => {
            expect((): void => {
                BasaltDataFilter.excludeByKeys({}, 'string' as any);
            }).toThrowError('The provided keys must be an array.');
        });

        it('should return an object without the excluded keys', (): void => {
            const data: { a: number; b: number; c: number } = { a: 1, b: 2, c: 3 };
            const result: { a: number; b: number } = BasaltDataFilter.excludeByKeys(data, ['c']);
            expect(result).toEqual({ a: 1, b: 2 });
        });

        it('should return the same object when no keys are excluded', (): void => {
            const data: { a: number; b: number; c: number } = { a: 1, b: 2, c: 3 };
            const result: { a: number; b: number; c: number } = BasaltDataFilter.excludeByKeys(data, []);
    ...
    describe('transformKeyToAnotherCase', (): void => {
        it('should throw an error for null data', (): void => {
            expect((): void => {
                BasaltDataFilter.transformKeyToAnotherCase(null as any, 'camel');
            }).toThrowError('The data object is required.');
        });

        it('should throw an error for undefined data', (): void => {
            expect((): void => {
                BasaltDataFilter.transformKeyToAnotherCase(undefined as any, 'camel');
            }).toThrowError('The data object is required.');
        });

        it('should throw an error if the data is not an object', (): void => {
            expect((): void => {
                BasaltDataFilter.transformKeyToAnotherCase('string' as any, 'camel');
        ...
        it('should throw an error for invalid caseType', (): void => {
            ...
            expect((): void => {
                BasaltDataFilter.transformKeyToAnotherCase(data, 'invalidCase' as any);
        ...
        it('should transform keys to camelCase', (): void => {
            ...
            const result: { 'ZIP-CODE': string; last_name: string; 'first-name': string } = BasaltDataFilter.transformKeyToAnotherCase(data, 'camel');
            expect(result).toEqual({ 'firstName': 'John', 'lastName': 'Doe', 'ZIPCODE': '12345' });
        });

        it('should transform keys to pascalCase', (): void => {
            const data: { 'ZIP-CODE': string; last_name: string; 'first-name': string } = { 'first-name': 'John', last_name: 'Doe', 'ZIP-CODE': '12345' };
            const result: { 'ZIP-CODE': string; last_name: string; 'first-name': string } = BasaltDataFilter.transformKeyToAnotherCase(data, 'pascal');
            expect(result).toEqual({ 'FirstName': 'John', 'LastName': 'Doe', 'ZIPCODE': '12345' });
        });
        ...
        it('should transform keys to snake_case', (): void => {
            ...
            const result: { firstName: string; ZIPCODE: string; LastName: string } = BasaltDataFilter.transformKeyToAnotherCase(data, 'snake');
            expect(result).toEqual({ 'first_name': 'John', 'last_name': 'Doe', 'zipcode': '12345' });
        });

        it('should transform keys to kebab-case', (): void => {
            const data: { firstName: string; ZIPCODE: string; LastName: string } = { firstName: 'John', LastName: 'Doe', ZIPCODE: '12345' };
            const result: { firstName: string; ZIPCODE: string; LastName: string } = BasaltDataFilter.transformKeyToAnotherCase(data, 'kebab');
    ...
    describe('filterByValue', (): void => {
        it('should throw an error for null data', (): void => {
            expect((): void => {
                BasaltDataFilter.filterByValue(null as any, (): boolean => true);
            }).toThrowError('The provided data object is either null or undefined.');
        });

        it('should throw an error for undefined data', (): void => {
            expect((): void => {
                BasaltDataFilter.filterByValue(undefined as any, (): boolean => true);
            }).toThrowError('The provided data object is either null or undefined.');
        });
        ...
        it('should throw an error if the callback is not a function', (): void => {
            ...
            expect((): void => {
                BasaltDataFilter.filterByValue(data, 'notAFunction' as any);
        ...
        it('should filter object properties based on value', (): void => {
            ...
            const result: { address: null; name: string; age: number } = BasaltDataFilter.filterByValue(data, (value: unknown): boolean => value !== null);
            expect(result).toEqual({
                name: 'John',
                age: 30
            });
        ...
        it('should return an empty object if all properties are filtered out', (): void => {
            ...
            const result: { name: null; age: null } = BasaltDataFilter.filterByValue(data, (value: unknown): boolean => value !== null);
            expect(result).toEqual({});
        });
    });
    describe('deepClone', (): void => {
        it('should throw an error for null data', (): void => {
            expect((): void => {
                BasaltDataFilter.deepClone(null as any);
            }).toThrow('The provided data object is either null or undefined.');
        });

        it('should throw an error for undefined data', (): void => {
            expect((): void => {
                BasaltDataFilter.deepClone(undefined as any);
            }).toThrow('The provided data object is either null or undefined.');
        });

        it('should clone primitive values', (): void => {
            const data: number = 42;
            const result: number = BasaltDataFilter.deepClone(data);
            expect(result).toEqual(data);
        });

        it('should clone Date objects', (): void => {
            const data: Date = new Date();
            const result: Date = BasaltDataFilter.deepClone(data);
            expect(result).toEqual(data);
        ...
        it('should clone RegExp objects', () => {
            ...
            const result: RegExp = BasaltDataFilter.deepClone(data);
            expect(result).toEqual(data);
            expect(result).not.toBe(data);
        });
        ...
        it('should clone arrays', (): void => {
            ...
            const result: (number | { a: string })[] = BasaltDataFilter.deepClone(data);
            expect(result).toEqual(data);
            expect(result).not.toBe(data);
        });
        ...
        it('should clone nested objects', (): void => {
            ...
            const result: { a: { b: { c: string }; e: string[] } } = BasaltDataFilter.deepClone(data);
            expect(result).toEqual(data);
